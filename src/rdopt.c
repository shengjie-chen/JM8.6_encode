 ...
 */
 void encode_one_macroblock ()
 {
   ...
   int         intra1 = 0;
   //为Islice或者是Pslice并且一定条件下，intra=1，只使用帧内预测
   ...
   if (!img->MbaffFrameFlag)//帧场自适应模式的判断
   {
   ...
   for (rerun=0; rerun<runs; rerun++)
   {
     ...
     // reset chroma intra predictor to default色度预测模式默认为平面模式
     ...
     if (!intra)
     {
       ...
       for (min_cost=1<<20, best_mode=1, mode=1; mode<4; mode++)
       {
         if (valid[mode])
         {
           ...
           {
             PartitionMotionSearch (mode, block, lambda_motion);//对范围内每一帧进行运动搜索，得到每帧运动向量和运动开销

             //--- set 4x4 block indizes (for getting MV) ---
             j = (block==1 && mode==2 ? 2 : 0);
             i = (block==1 && mode==3 ? 2 : 0);

             //--- get cost and reference frame for forward prediction ---最佳参考帧的选择
             ...
             if (bframe)
             {
               ...
               //--- get prediction direction ---- 0 双前 ；1 双后 ；2 双向
...
//          else cost8x8 += min_rdcost;          
    ...
    if (input->rdopt)
    {
      ...
      // precompute all new chroma intra prediction modes 预计算所有新的色度内预测模式
